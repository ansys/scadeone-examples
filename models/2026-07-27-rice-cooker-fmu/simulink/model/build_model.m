...
% required by built-in/FMU: folder must be on path
addpath(fullfile(thisDir, '..', '..', 'fmu-controller'));
addpath(fullfile(thisDir, '..', '..', 'fmu-panel'));
...
% Unit Delay on tempC: explicit sensor lag matching FMU 200 ms cycle.
% Provides current temperature (not stale integrator state) with transparent delay.
add_block('simulink/Discrete/Unit Delay', [modelName '/TempC_Delay'], ...
    'SampleTime', '0.2', ...
    'InitialCondition', '20', ...  % Initial pot temp = 20°C
    'Position', [150 220 190 240]);
...
%   port 2: tempPot_Controller   <- RicePhysics tempC (with 200ms delay)
%   port 3: btnStartStop
%   port 4: btnDelay
%   port 5: btnSetTime
add_line(modelName, 'isLidOpen/1',    'Controller/1', 'autorouting','on');
add_line(modelName, 'RicePhysics/4',  'TempC_Delay/1', 'autorouting','on');  % Direct tempC
add_line(modelName, 'TempC_Delay/1',  'Controller/2', 'autorouting','on');     % Delayed tempC
